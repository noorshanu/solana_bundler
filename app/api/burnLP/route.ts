...
export async function POST(request: Request) {
  ...
  try {
    ...
    const settings: UserSetting | any = await prisma.userSetting.findFirst({
      where: { userId: userId },
    });
    const tokenInfo: any = await prisma.tokenMetadata.findFirst({
      where: { userId: userId, poolId: poolId },
    });

    if (settings && tokenInfo) {
      const tokenLauncherKP = settings?.launchPK;
      const rpcUrl = settings?.rpcURL;
      const tokenLauncherKeypair = Keypair.fromSecretKey(
        bs58.decode(tokenLauncherKP)
      );
      const connection = settings.enableDevnet
      ...
      const PROGRAMIDS = settings.enableDevnet
        ? DEVNET_PROGRAM_ID
        : MAINNET_PROGRAM_ID;
      ...
      const IXinstructions = ComputeBudgetProgram.setComputeUnitPrice({
        microLamports: settings.priorityFee * LAMPORTS_PER_SOL,
      });
      ...
      const signatureStatuses = await connection.getSignatureStatuses([status]);
